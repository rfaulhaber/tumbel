;;; tumbel-post.el --- Post Org files and entries  -*- lexical-binding: t; -*-
...
;; `tumbel-post-buffer' and `tumbel-post-subtree' send Org text that
...
(defun tumbel-post--subtree-body ()
...
(defun tumbel-post--record-keywords (id blog)
...
(defun tumbel-post--send (blog id plist preview record)
...
post as `tumbel-compose-parse' produces them.  With PREVIEW, show the
request instead of sending it.  RECORD is called with the id of a
created post."
  (let* ((body (tumbel-compose--request-body plist))
         (blocks (length (alist-get 'content (car body)))))
    (when (zerop blocks)
      (user-error "The post is empty"))
    (cond
     (preview (tumbel-compose-preview-request blog body))
     ((y-or-n-p (if id
      ...
      (cl-flet ((done (response)
        ...
        (if id
            (tumbel-api-edit-post blog id (car body) :files (cdr body)
                                  :then #'done :else #'failed)
          (tumbel-api-create-post blog (car body) :files (cdr body)
                                  :then #'done :else #'failed)))))))

;;;###autoload
(defun tumbel-post-buffer (&optional preview)
...
request instead of sending it."
  (interactive "P")
  (tumbel-post--check-buffer)
  (let* ((keywords (org-collect-keywords tumbel-post-keywords))
         (blog (tumbel-post--keyword keywords "BLOG"))
         (state (tumbel-post--keyword keywords "STATE"))
         (buffer (current-buffer)))
    (tumbel-post--send
     (or blog (tumbel-user-default-blog))
     (tumbel-post--keyword keywords "TUMBLR_ID")
     (list :tags (tumbel-post--file-tags keywords)
           :state (and state (downcase state))
           :publish-on (tumbel-post--keyword keywords "PUBLISH_ON")
           :body (tumbel-post--buffer-body
                  (tumbel-post--keyword keywords "TITLE")))
     preview
     (lambda (id)
       (when (buffer-live-p buffer)
         (with-current-buffer buffer
           (tumbel-post--record-keywords
            id (and (null blog) (tumbel-user-default-blog)))))))))

;;;###autoload
(defun tumbel-post-subtree (&optional preview)
  ...
  (save-excursion
    ...
    (let* ((keywords (org-collect-keywords tumbel-post-keywords))
           (blog (or (org-entry-get nil "TUMBLR_BLOG" t)
                     (tumbel-post--keyword keywords "BLOG")))
           (state (or (org-entry-get nil "TUMBLR_STATE" t)
                      (tumbel-post--keyword keywords "STATE")))
           (marker (point-marker)))
      (tumbel-post--send
       (or blog (tumbel-user-default-blog))
       ...
       (list :tags (mapcar #'substring-no-properties (org-get-tags))
             ...
             :publish-on (or (org-entry-get nil "TUMBLR_PUBLISH_ON" t)
                             (tumbel-post--keyword keywords "PUBLISH_ON"))
             :body (tumbel-post--subtree-body))
       preview
       (lambda (id)
